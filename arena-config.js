...
(function () {
  ...
  function applyConfig(cfg) {
    ...
    for (var k = 0; k < emailSwapEls.length; k++) {
      ...
      if (cfg.hasOwnProperty(eKey)) {
        ...
        var updated  = existing.replace(/mailto:[^?#]*/i, 'mailto:' + cfg[eKey]);
        eEl.setAttribute('href', updated);
      } else {
  ...
  /**
   * Fetch failed — hide every config-dependent element to avoid stale data.
   */
  function hideAll() {
    var selectors = [
      '[' + ATTR_TEXT + ']',
